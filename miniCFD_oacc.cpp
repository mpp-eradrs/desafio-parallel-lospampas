...
#pragma acc parallel loop collapse(2) vector_length(128) independent cache(flux) present(state[0:size_state], flux[0:(nnx+1)*(nnz+1)*NUM_VARS], cfd_dens_theta_cell[0:nnz+2*hs],cfd_dens_cell[0:nnz+2*hs]) async(0)
  for (int k=0; k<nnz+1; k++) {
    for (int i=0; i<nnx; i++) {
	  ...
	  int k2 = (k+2)*aux2+ihs;
	  int k3 = (k+3)*aux2+ihs;
          double stencil0 = state[k0];
          double stencil1 = state[k1];
          double stencil2 = state[k2];
          double stencil3 = state[k3];
          double vals0 = -stencil0/12 + 7*stencil1/12 + 7*stencil2/12 - stencil3/12;
          double d_vals0 = -stencil0 + 3*stencil1 - 3*stencil2 + stencil3;
          stencil0 = state[aux1+k0];
          stencil1 = state[aux1+k1];
          stencil2 = state[aux1+k2];
          stencil3 = state[aux1+k3];
          double  vals1 = -stencil0/12 + 7*stencil1/12 + 7*stencil2/12 - stencil3/12;
          double  d_vals1 = -stencil0 + 3*stencil1 - 3*stencil2 + stencil3;
          stencil0 = state[2*aux1+k0];
          stencil1 = state[2*aux1+k1];
          stencil2 = state[2*aux1+k2];
          stencil3 = state[2*aux1+k3];
          double vals2 = -stencil0/12 + 7*stencil1/12 + 7*stencil2/12 - stencil3/12;
	  double d_vals2 = -stencil0 + 3*stencil1 - 3*stencil2 + stencil3;
          stencil0 = state[3*aux1+k0];
          stencil1 = state[3*aux1+k1];
          stencil2 = state[3*aux1+k2];
          stencil3 = state[3*aux1+k3];
          double vals3 = -stencil0/12 + 7*stencil1/12 + 7*stencil2/12 - stencil3/12;
          double d_vals3 = -stencil0 + 3*stencil1 - 3*stencil2 + stencil3;
      //Compute density, u-wind, w-wind, potential temperature, and pressure (r,u,w,t,p respectively)
